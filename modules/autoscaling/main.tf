# modules/autoscaling/main.tf - Auto scaling configuration for ECS services
...
# Request count per target scaling policy (if ALB is used)
...
# Scheduled scaling for predictable load patterns (optional)
resource "aws_appautoscaling_scheduled_action" "scale_up" {
  count              = length(var.scheduled_actions) > 0 ? length(var.scheduled_actions) : 0
  name               = "${var.project_name}-${var.environment}-scale-${count.index}"
  service_namespace  = aws_appautoscaling_target.ecs_target.service_namespace
  resource_id        = aws_appautoscaling_target.ecs_target.resource_id
  scalable_dimension = aws_appautoscaling_target.ecs_target.scalable_dimension
